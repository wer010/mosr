...
class SMPLSequence(Node):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.poses_head = to_torch(poses_head, dtype=dtype, device=device)
        ...
        if smpl_layer.model_type == "smpl":
            full_pose = torch.cat([self.poses_root, self.poses_body], dim=-1)
        elif smpl_layer.model_type == "smplh":
            full_pose = torch.cat([self.poses_root, self.poses_body, self.poses_left_hand, self.poses_right_hand], dim=-1)
        else:
            full_pose = torch.cat([self.poses_root, self.poses_body, self.poses_left_hand, self.poses_right_hand, self.poses_head], dim=-1)
    ...
    def from_amass(
        ...
        **kwargs,
        ...
        if smpl_layer is None:
            if body_data["poses"].shape[1]==72:
                model_type = 'smpl'
            elif body_data["poses"].shape[1] == 156:
                model_type = 'smplh'
            elif body_data["poses"].shape[1] == 165:
                model_type = 'smplx'
            else:
                model_type = body_data['surface_model_type'].item() if 'surface_model_type' in body_data else 'smplx'
            smpl_layer = SMPLLayer(model_type=model_type, gender=body_data["gender"].item(), device=C.device)

        f_key = [key for key in body_data.keys() if 'frame' in key and 'rate' in key]
        assert len(f_key)==1
        ...
        if log:
            ...
            print("FPS {}".format(body_data[f_key[0]]))
        ...
        if fps_out is not None:
            fps_in = body_data[f_key[0]].tolist()
            if fps_in != fps_out:
    ...
    def fk(self, current_frame_only=False):
        ...
        if current_frame_only:
            ...
            poses_head = self.poses_head
            trans = self.trans
            betas = self.betas

        if self.smpl_layer.model_type == "smpl":
            ...
            verts, joints = self.smpl_layer(
                ...
                poses_left_hand=poses_left_hand,
                poses_right_hand=poses_right_hand,
                poses_jaw = poses_head[:,0:3],
                poses_leye = poses_head[:,3:6],
                poses_reye = poses_head[:,6:9],
                betas=betas,
                trans=trans,
            )
        ...
        skeleton = (
            self.smpl_layer.skeletons()["all"].T
        )
    ...
    def redraw(self, **kwargs):
        ...
        if current_frame_only:
            ...
            if self.smpl_layer.model_type == "smpl":
                ...
                full_pose = torch.cat([self.poses_root, self.poses_body, self.poses_left_hand, self.poses_right_hand],
                                      dim=-1)
            else:
                full_pose = torch.cat(
                    [self.poses_root, self.poses_body, self.poses_left_hand, self.poses_right_hand, self.poses_head],
                    dim=-1)
            global_oris = local_to_global(
                full_pose,
                self.skeleton[:, 0],
    ...
    def add_frames(self, poses_body, poses_left_hand = None,
                   poses_right_hand = None, poses_head = None,
                   poses_root=None, trans=None):
        # Append poses_body.
        ...
        if poses_left_hand is None:
            poses_left_hand = torch.zeros([len(poses_body), self.poses_left_hand.shape[1]])
        elif len(poses_left_hand.shape) == 1:
            poses_left_hand = poses_left_hand[np.newaxis]
        ...
        if poses_right_hand is None:
            poses_right_hand = torch.zeros([len(poses_body), self.poses_right_hand.shape[1]])
        elif len(poses_right_hand.shape) == 1:
            poses_right_hand = poses_right_hand[np.newaxis]
        self.poses_right_hand = torch.cat((self.poses_right_hand, to_torch(poses_right_hand, self.dtype, self.device)))

        if poses_head is None:
            poses_head = torch.zeros([len(poses_body), self.poses_head.shape[1]])
        elif len(poses_head.shape) == 1:
            poses_head = poses_head[np.newaxis]
        self.poses_head = torch.cat((self.poses_head, to_torch(poses_head, self.dtype, self.device)))

        self.n_frames = len(self.poses_body)
        self.redraw()

    def update_frames(self, poses_body, frames, poses_left_hand = None,
                   poses_right_hand = None, poses_head = None, poses_root=None, trans=None, betas=None):
        ...
        if poses_left_hand is not None:
            poses_left_hand[frames] = to_torch(poses_left_hand, self.dtype, self.device)
        if poses_right_hand is not None:
            poses_right_hand[frames] = to_torch(poses_right_hand, self.dtype, self.device)
        if poses_head is not None:
            poses_head[frames] = to_torch(poses_head, self.dtype, self.device)
        self.redraw()
    ...
    def remove_frames(self, frames):
        ...
        self.poses_left_hand = self.poses_left_hand[frames_to_keep]
        self.poses_right_hand = self.poses_right_hand[frames_to_keep]
        self.poses_head = self.poses_head[frames_to_keep]
