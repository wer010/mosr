...
class StageOneFitter(torch.nn.Module):
    def __init__(self,
        ...
        self.device = device
        marker_pose = torch.zeros()
        self.marker_pose = torch.nn.Parameter()
...
def main():
    ...
    for ind in range(len(dataset)):
        ...
        # prepare data for stage 1 training
        ids = subject_ids[stage2_data['subject_name']]
        random.shuffle(ids)
        stage1_data = []
        for i in ids:
            data = dataset[i]
            frame_ind = np.random.randint(0, data['markers'].shape[0], 3)
            stage1_data.append({'data': data['markers'][frame_ind],
                                'label':data['sequence_labels'],
                                'id':data['id']})
            if len(stage1_data)>=(12/3):
                break
